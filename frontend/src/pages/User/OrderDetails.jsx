...
import { FiPrinter, FiTruck, FiExternalLink } from 'react-icons/fi';
...
const OrderDetails = () => {
  ...
  const handleTrackOrder = async () => {
    ...
    try {
      const { data } = await axios.get(`${API_URL}/orders/${id}/track`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });

      if (data.success) {
        setTrackingData(data.tracking);
        setOrder(data.order); // Update order with latest status
        toast.success('Tracking information updated');
      } else {
        toast.error(data.message || 'Failed to fetch tracking information');
        // Still show existing tracking data if available
        if (order.shipmozoAwb) {
          setTrackingData({
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {order.shipmozoAwb && (
                        <button
                          ...
                          className="px-3 py-1.5 text-xs font-semibold bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                    ...
                    {order.shipmozoAwb ? (
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-sm font-semibold text-gray-900">{order.shipmozoAwb}</span>
                        </div>
                        {order.courierName && (
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-600">Courier:</span>
                            <span className="text-sm font-semibold text-gray-900">{order.courierName}</span>
                          </div>
                        )}
                        {(trackingData?.status || order.shipmentStatus) && (
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-600">Status:</span>
                            <span className={`text-xs px-2 py-1 rounded-full font-semibold ${
                              (trackingData?.status || order.shipmentStatus) === 'delivered' ? 'bg-green-100 text-green-700' :
                              (trackingData?.status || order.shipmentStatus) === 'in_transit' ? 'bg-blue-100 text-blue-700' :
                              (trackingData?.status || order.shipmentStatus) === 'out_for_delivery' ? 'bg-purple-100 text-purple-700' :
                              'bg-gray-100 text-gray-700'
                            }`}>
                              {(trackingData?.status || order.shipmentStatus || '').replace('_', ' ').toUpperCase()}
                            </span>
                          </div>
                        )}
                        {(trackingData?.tracking_url || order.trackingUrl) && (
                          <a
                            href={trackingData?.tracking_url || order.trackingUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-2 mt-3 px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 transition-colors text-sm font-semibold"
                          >
                            ...
                            <div className="space-y-2 max-h-32 overflow-y-auto">
                              {trackingData.events.map((event, idx) => (
                                <div key={idx} className="text-xs text-gray-600">
                                  <span className="font-semibold">{event.status || event.description}</span>
                      ...
                      <div className="text-sm text-amber-600">
                        ⏳ Shipment creation is pending. It will be processed shortly.
                      </div>
                    ) : null}
                  </div>
                )}
